...
YYin=$DIR1/BLM_ICs.f 
#
#                                  First, the system attains it's SS for 1020 s.
rm *.o; rm *.dat; make; ./main
#
#                                  Non Steady-State (Do not cancel j8 from now on)
#var_SS=$(grep "fac=" $input1)
#var_NSS="${var_SS/0.0d0/1.0d0}" 
#sed -i -e "s/$var_SS/$var_NSS/" $input1
#                                  Oxidative (OCF) and glycolytic (GCF) pulse till 1040 s. 
var_tIn=$(grep "tend=" $input2)
var_tOut="${var_tIn/1.02d+3/2.0d+1}" 
...
rm *.o; make; ./main
#
#                                  Change on glucolytic (GCF) pulse till 1060 s.
...
rm *.o; make; ./main
#
#                                  Take out one part of oxidative (OCF) pulse till 1140 s.
...
rm *.o; make; ./main
#
#                                  Add a part on oxidative (OCF) pulse till 1240 s.
...
rm *.o; make; ./main
#
#                                  Take out a part of oxidative (OCF) pulse till 1320 s.
...
rm *.o; make; ./main
#
#                                  Return to the initial values till 1340 s.
...
rm *.o; make; ./main
#
#                                  Oxidative (OCF) and Glycolytic (GCF) pulse till 1460 s
...
rm *.o; make; ./main
#
#                                  Take out the Glycolytic (GCF) pulse and change Oxidative (OCF) till 1540 s
...
var_tOut3="${var_tOut2/1.2d+2/1.46d+3}" 
#
# Change paramet.i and SUB_main.f
